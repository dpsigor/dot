...
require('lazy').setup({
  ...
  { -- Fugitive
    'tpope/vim-fugitive',
    config = function()
      vim.keymap.set('n', '<leader>g', '<cmd>G<CR><C-w>o', { desc = 'Open [G]it' })
    end,
  },
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>fz', function()
        builtin.grep_string {
          cwd = '~/repos/github.com/dpsigor/zet',
    ...
    opts = {
      ...
      textobjects = {
        ...
        move = {
          ...
          goto_previous_start = {
            ['[['] = { query = '@function.outer', desc = 'Previous funtion start' },
          },
        },
      },
    },
...
-- Salvar com :w e fechar com :q
vim.keymap.set('n', '<leader>q', ':q<CR>', { silent = true, noremap = true })
vim.keymap.set('n', '<leader>w', ':w<CR>', { silent = true, noremap = true })

-- Limpar highlight com <leader><CR>
vim.keymap.set('n', '<leader><CR>', ':nohl<CR>', { silent = true, noremap = true })

-- Não yank ao colar em visual mode
...
vim.cmd [[
  augroup JumpToLastLocation
    autocmd!
    autocmd BufReadPost * if line("'\"") > 0 && line("'\"") <= line("$") | exe "normal! g`\"" | endif
  augroup END
]]

-- Manter no modo visual ao identar linhas
vim.keymap.set('v', '<', '<gv')
vim.keymap.set('v', '>', '>gv')

-- Substituir palavras sob o cursor
vim.keymap.set('n', '<leader>s', '*<S-n>cgn', { silent = true, noremap = true })
